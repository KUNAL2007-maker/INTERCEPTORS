...
* **✅ Tasks Done (Completed)**:
  ...
  * [x] Verified the **entire pipeline end-to-end on live blockchain data** and stripped every silent demo fallback. Traced a real public wallet (`0x4aaa…cb21`) → `source:live`, 233 real transfers, 5 hops, attributed to **Binance** (`le@binance.com`, confidence 92); XGBoost + real NetworkX GDS + genuine A\* (`f=g+h`) all firing; **Gemini Live integrated** on the exact `models/gemini-3.1-flash-live-preview`. `traceWallet` is now live-first (`emptyLiveTrace` returns an honest empty/degraded result, never the fabricated mock scenario); `/api/trace`, NCRP ingest and the exchange freeze-response flow no longer paper over failures with fake data; the "Simulated LEA Environment" banner is removed (labelled "Load demo case" sample retained by product decision). `next build` clean; all 9 core flows PASS. Committed `c645a08`.
  * [x] Closed **4 forensic-integrity defects** and verified them live end-to-end (commit `e33a2e1`): (1) **VASP action-reporting deadlock** — decoupled `action` (`"report_action"`) from `action_taken` (`"FREEZE_EXECUTED"`) across the ExchangePortal UI, `/api/notices` and `db.recordVaspResponse`, so the payload spread no longer overwrites the routing action; (2) **statutory immutability** — `recordVaspResponse` now rejects a repeat freeze report once `action_reported_at` is set (HTTP 400), making a certified action write-once; (3) **root-seed VASP/mixer attribution** — `liveTrace`/`emptyLiveTrace` attribute the hop-0 seed immediately (a mixer seed → `BURNER_MULE`, a verified VASP seed → `VASP_DEPOSIT`, instead of a null-VASP `VICTIM_ENTRY`); (4) **audit API alias** — `GET /api/audit` now returns `audit_logs` alongside `logs`, and the e2e/security fixtures were synced to real credentials + case numbers. **120 automated assertions (70 security + 50 e2e) PASS, 0 FAIL**, plus direct live seed-attribution probes on the Tornado Cash and Binance hot wallets (`source:live`, hop-0 attributed, confidence 96/92). `tsc --noEmit` clean.
  * [x] Completed the **full 18-point role-based UI restructure** (court-honest, demo-ready). Every view now shows only what its role should touch and every workflow moves one clicked case forward: (1) victim-name field in complaint filing; (2) supervisor triage → clickable docket → `SupervisorCaseDetail` modal (amount, complainant, suspect wallet, read-only case-flow timeline, assign-to-field-officer) with (3) dark-theme readable officer/priority dropdowns; (4) supervisor stripped to allocation + read-only flow (no fund-flow/canvas/audit/register); (5)+(7) crime canvas & live monitor hidden from every tab, standalone trace removed from the field IO; (6)+(16) `GraphView` shows only the traced wallet under a case-aware "Money-Flow Graph · <case> · <complainant> · suspect <wallet>" heading, fitted to the pane by a ResizeObserver so it never overflows; (8) the Case Assistant re-traces the *selected* case before answering (store `runTrace` now returns the fresh `TraceResult`); (9) field IO's draft is scoped to the selected case, headed "Draft Requisition — UNSIGNED", with Edit + "Send to Gazetted Officer" and no signature-verify surface; (10) gazetted nav is the Statutory Review Desk only; (11) gazetted opens the same money-flow graph as the SI; (12) sign-securely → "Forward to Compliance Officer", audit trail removed; (13) compliance desk gates on signature-verify, then offers "Freeze in Escrow" + "Send Acknowledgement" (freeze opens the acknowledgement; acknowledgement routes to the court); (14) court reviewer gets "View Acknowledgement" + "Export Sec 65B Certificate" to **Word and PDF** on the single clicked case, with fund-flow/canvas/chain-of-custody removed; (15) national analyst reduced to National Correlation only (address-lookup, graph, canvas & audit permissions and audit-route access stripped); (17)+(18) demo cases are user-loaded only, the showcase trace being a deterministic medium graph fanning into four verified exchanges (WazirX/Binance/CoinDCX/Kraken) + a Tornado Cash mixer. Verified live: `tsc --noEmit` + `next build` clean, **73 security + 54 e2e assertions PASS, 0 FAIL**, all 7 personas resolve to the right role/gazetted flag, and the demo graph renders inside the viewport with the case-aware heading.
* **🔄 Tasks Working On (In Progress)**:
